#!/usr/bin/env python
# COPYRIGHT 2018 AGPL3 Brecht Machiels
...
def build(
    builder, success_msg=None, extra_opts=None, outdir=None,
    doctrees=True,
):
...
def htmlhelp():
    ...
    build(
        'htmlhelp', 'Now you can run HTML Help Workshop with the .hhp '
        'project file in {}.',
    )
...
def qthelp():
    ...
    return build(
        'qthelp', 'Now you can run "qcollectiongenerator" with the '
...
def texinfo():
    """make Texinfo files"""
    return build(
        'texinfo', 'The Texinfo files are in {}.\n'
...
def gettext():
    ...
    return build(
        'gettext', 'The message catalogs are in {}.', outdir='locale',
        doctrees=False,
    )
...
def linkcheck():
    ...
    return build(
        'linkcheck', 'Look for any errors in the above output or in '
        '{}/output.txt.',
    )
